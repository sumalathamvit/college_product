...
function ManagementReport() {
  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      var csvData = [
        ["No.", "Acc No.", "Title", "Author", "YOP", "Edition", "Unit Price"],
      ];

      if (type == 1) {
        values.map((item, index) => {
          index > 0 && data[index - 1].subject_name === item.subject_name
            ? (totalQty += item.unit_price)
            : (totalQty = item.unit_price);
          if (
            index === 0 ||
      ...
      if (type == 1) {
        ...
        preFunction.generatePDF(collegeName, "Management Report", csvData);
      } else {
        preFunction.downloadCSV(csvData, "Management Report");
      }
      setLoad(false);
    } catch (error) {
      console.log(error);
  ...
  const getBookDetailList = async (values, showAll = 0, report = 0) => {
    ...
    try {
      setLoad(true);
      const getBookDetail = await libraryApi.getManagementReportSearch(
