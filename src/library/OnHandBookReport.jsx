...
import { onHandBookReportList } from "../component/common/CommonArray";
...
function OnHandBookReport() {
  let totalQty = 0;
  const [load, setLoad] = useState(false);
  const [data, setData] = useState([]);
  const [showLoadMore, setShowLoadMore] = useState(false);

  const [reportType, setReportType] = useState(onHandBookReportList[0]);
  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (reportType.key == 1 || reportType.key == 2) {
        if (type == 1) {
          var pdfData = [[{ content: "No." }]];
          if (reportType.key == 1) {
            pdfData[0].push({ content: "Acc No." });
          }
          pdfData[0].push(
            { content: "Title" },
            { content: "Author Name" },
          ...
          values.map((item, index) => {
            ...
            if (reportType.key == 1) {
              pdfData[index + 1].push({ content: item.access_number });
            }
            pdfData[index + 1].push(
              {
            ...
            if (reportType.key == 2) {
              pdfData[index + 1].push({ content: item.quantity });
            }
          });
          ...
          var csvData = [["No."]];
          if (reportType.key == 1) {
            csvData[0].push("Access No.");
          }
          csvData[0].push("Title", "Author Name", "Subject", "Year", "Edition");
          if (reportType.key == 2) {
            csvData[0].push("Qty");
          }
          values.map((item, index) => {
            ...
            if (reportType.key == 1) {
              csvData[index + 1].push(item.access_number);
            }
            ...
            if (reportType.key == 2) {
              csvData[index + 1].push(item.quantity);
            }
          });
      ...
      if (reportType.key == 3) {
        if (type == 1) {
          ...
          var columnWidth = [5, 40, 30, 10, 10, 5];
        } else {
          // TypeError: row.join is not a function
          var csvData = [
            ["No.", "Title", "Author Name", "Year", "Edition", "Qty"],
          ];
          values.map((item, index) => {
            index > 0 && data[index - 1].subject_name === item.subject_name
              ? (totalQty += item.quantity)
      ...
      if (reportType.key == 4) {
        if (type == 1) {
          ...
          var columnWidth = [5, 10, 40, 30, 5, 10];
        } else {
          var csvData = [
            ["No.", "Access No.", "Title", "Author Name", "Year", "Edition"],
          ];
          values.map((item, index) => {
            if (
              index === 0 ||
  ...
  const getBookDetailList = async (values, showAll = 0, report = 0) => {
    ...
    try {
      ...
      const onHandBookRes = await libraryApi.getOnHandBook(
      ...
      console.log("onHandBookRes-------", onHandBookRes);
      if (report) {
        handleCSVData(onHandBookRes.data.message, report);
      } else {
        setData(onHandBookRes.data.message);
        setShowLoadMore(false);
        if (onHandBookRes.data.message.length === string.PAGE_LIMIT) {
                    ...
                    <>
                      <div className="row mt-4 border p-3">
                        ...
                        <div className="table-responsive mt-4 p-0">
                          {reportType.key == 1 || reportType.key == 2 ? (
                            <table
...
export default OnHandBookReport;
