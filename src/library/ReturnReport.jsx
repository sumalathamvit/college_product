...
import { libraryReturnReportList } from "../component/common/CommonArray";
...
function LibraryBookReturnReport() {
  const [load, setLoad] = useState(false);
  const [data, setData] = useState([]);
  const [showLoadMore, setShowLoadMore] = useState(false);

  const [reportType, setReportType] = useState(libraryReturnReportList[0]);
  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (
        reportType.value == "return_day_wise" ||
        reportType.value == "return_week_wise" ||
        reportType.value == "return_month_wise"
      ) {
        if (type == 1) {
          var pdfData = [
            [
          ...
          values.map((item1, index1) => {
            ...
            if (reportType.value == "return_day_wise") {
              returnRow.content =
                "Return Date : " +
                moment(item1.return_date).format("DD-MM-YYYY");
            } else if (reportType.value == "return_week_wise") {
              returnRow.content =
                "Return Week : " +
                moment(item1.return_date).format("DD-MM-YYYY") +
                " to " +
                moment(item1.return_date).add(6, "days").format("DD-MM-YYYY");
            } else if (reportType.value == "return_month_wise") {
              returnRow.content =
                "Return Month : " +
                moment(item1.return_date).format("MMM-YYYY");
            }
          ...
          values.map((item1, index1) => {
            ...
            const returnRow = [];
            returnRow[0] =
              "Return Date : " + moment(item1.return_date).format("DD-MM-YYYY");
            for (let i = 1; i < 10; i++) {
      ...
      if (
        reportType.value == "return_student_day_wise" ||
        reportType.value == "return_student_week_wise" ||
        reportType.value == "return_student_month_wise"
      ) {
        if (type == 1) {
          var pdfData = [
            [
          ...
          values.map((item1, index1) => {
            ...
            if (reportType.value == "return_student_day_wise") {
              returnRow.content =
                "Return Date : " +
                moment(item1.return_date).format("DD-MM-YYYY");
            } else if (reportType.value == "return_student_week_wise") {
              returnRow.content =
                "Return Week : " +
                moment(item1.return_date).format("DD-MM-YYYY") +
                " to " +
                moment(item1.return_date).add(6, "days").format("DD-MM-YYYY");
            } else if (reportType.value == "return_student_month_wise") {
              returnRow.content =
                "Return Month : " +
                moment(item1.return_date).format("MMM-YYYY");
            }
      ...
      if (
        reportType.value == "return_staff_day_wise" ||
        reportType.value == "return_staff_week_wise" ||
        reportType.value == "return_staff_month_wise"
      ) {
        if (type == 1) {
          var pdfData = [
            [
          ...
          values.map((item1, index1) => {
            ...
            if (reportType.value == "return_staff_day_wise") {
              returnRow.content =
                "Return Date : " +
                moment(item1.return_date).format("DD-MM-YYYY");
            } else if (reportType.value == "return_staff_week_wise") {
              returnRow.content =
                "Return Week : " +
                moment(item1.return_date).format("DD-MM-YYYY") +
                " to " +
                moment(item1.return_date).add(6, "days").format("DD-MM-YYYY");
            } else if (reportType.value == "return_staff_month_wise") {
              returnRow.content =
                "Return Month : " +
                moment(item1.return_date).format("MMM-YYYY");
            }
  ...
  const handleDayWise = (messages, report) => {
    ...
    for (const dayKey in uniqueDays) {
      filterList.push({
        return_date: moment(dayKey).format("YYYY-MM-DD"),
  ...
  const handleWeekWise = (messages, report) => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (!uniqueWeeks[weekKey]) {
        uniqueWeeks[weekKey] = {
          week: weekKey,
          return_date: weekStartDate, // Assign the start date of the week
          books: [],
        };
      }
                  ...
                  <div className="row no-gutters">
                    <div className="col-lg-6 pe-2">
                      ...
                      <ReactSelectField
                        ...
                        options={libraryReturnReportList}
                        onChange={(text) => {
                          setReportType(text);
                          handleClear();
                        }}
...
export default LibraryBookReturnReport;
