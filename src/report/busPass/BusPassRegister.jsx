  ...
  const handleCSVData = async (exportData, values, type) => {
    try {
      ...
      if (type == 2) {
        ...
        preFunction.downloadCSV(csvData, "Student Bus Pass Register");
      } else {
        let pdfData = [];
        var filterContent = [];
          ...
          filterContent.push([
            {
              content:
                (values.enrollNo
                  ...
                  : "") +
                (values.fromDate
                  ? `   From Dat : ${moment(values.fromDate).format(
                      "DD-MM-yyyy"
                    )}`
                  : "") +
                (values.toDate
        ...
        exportData.map((item, index) => {
          pdfData.push([
            ...
            item.courseName.substr(0, 31) + " / " + item.className,
            item.passRegisterDate
              ? moment(item.passRegisterDate).format("DD-MM-yyyy")
              : null,
            item.boardingPlace.substr(0, 35),
            item.passNo,
            item.amount,
            "",
            "",
...
export default BusPassRegister;
