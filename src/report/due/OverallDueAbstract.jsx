...
function OverallDueAbstract() {
  ...
  const handleDueAbstractCSVData = async (values, report) => {
    ...
    if (report === 1) {
      ...
      preFunction.downloadCSV(csvData, "Student Overall Due Abstract.csv");
    }
    setLoad(false);
  };
  ...
  const handleShow = async (values, showAll = 0, report = 0) => {
    ...
    try {
      ...
      dueOverallAbstractRes.data.message.due_abstract.map((item, index) => {
        ...
        concessionTotalSum += item.concessionAmount ? item.concessionAmount : 0;
...
export default OverallDueAbstract;
