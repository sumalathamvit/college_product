...
function LateArrivalReport() {
  ...
  const handleCSVData = async (exportData, type, values) => {
    ...
    preFunction.downloadCSV(csvData, "Late Arrival Report");
    return;
  };
  const handleShow = async (values, report) => {
    ...
    try {
      setLoad(true);
      setShowRes(true);
      const getLateArrivalListRes = await employeeApi.getLateArrivalList(
      ...
      console.log("getLateArrivalListRes---", getLateArrivalListRes);

      setData(getLateArrivalListRes.data.message.data.late_arrival_detail);
      setLoad(false);
    } catch (error) {
      setLoad(false);
      console.log(error);
