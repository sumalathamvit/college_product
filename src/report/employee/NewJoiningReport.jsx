...
function NewJoiningReport() {
  ...
  const handleCSVData = async (exportData, type, values) => {
    ...
    preFunction.downloadCSV(csvData, "Employee New Joining Report");
    return;
  };
  ...
  const handleShow = async (values, showAll = 0, report = 0) => {
    ...
    try {
      setLoad(true);
      const getNewlyJoiningListRes = await employeeApi.getNewJoiningReport(
      ...
      console.log("getNewlyJoiningListRes---", getNewlyJoiningListRes);
      setData(getNewlyJoiningListRes.data.message.data.newly_joining_report);
      if (showAll === 0) {
        if (
          getNewlyJoiningListRes.data.message.data.newly_joining_report.length >
          string.PAGE_LIMIT
        )
      ...
      if (report == 1 || report == 2) {
        handleCSVData(
          getNewlyJoiningListRes.data.message.data.newly_joining_report,
          report,
          values
        );
      }
...
export default NewJoiningReport;
