...
function DepartmentBookBySubjectWise() {
    ...
    try {
      ...
      if (type == 1) {
        ...
        preFunction.generatePDFContent(
          ...
          "Department Book Detail Subject Wise",
          pdfHeadToPass,
          pdfDataToPass,
          colWidthToPass
          // "Landcape"
        ...
        var csvData = [["No.", "Acc No.", "Title", "Author", "Edition", "YOP"]];
        values.map((item, index) => {
          if (
            index === 0 ||
            values[index - 1].subject_name !== item.subject_name
        ...
        preFunction.downloadCSV(csvData, "Department Book Detail Subject Wise");
      }
      setLoad(false);
    } catch (error) {
      console.log(error);
...
export default DepartmentBookBySubjectWise;
