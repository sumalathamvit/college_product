...
import { departmentBookReportList } from "../../component/common/CommonArray";
...
function DepartmentBookDetailReport() {
  const [load, setLoad] = useState(false);
  const [data, setData] = useState([]);
  const [showLoadMore, setShowLoadMore] = useState(false);

  const [reportType, setReportType] = useState(departmentBookReportList[0]);
  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (reportType.value == "book_detail_by_access_number") {
        var csvData = [
        ...
        var columnWidths = [3, 7, 38, 20, 20, 7, 5];
      }
      ...
      if (reportType.value == "book_detail_by_subject") {
        if (type == 1) {
          ...
          var columnWidths = [3, 7, 48, 30, 5, 7];
        } else {
          var csvData = [
            ["No.", "Acc No.", "Title", "Author", "YOP", "Edition"],
          ];
          values.map((item, index) => {
            if (
              index === 0 ||
      ...
      if (reportType.value == "book_title_wise_qty") {
        if (type == 1) {
          var csvData = [
            [
              { content: "No." },
          ...
          var columnWidths = [3, 50, 30, 5, 7, 5];
        } else {
          var csvData = [["No.", "Title", "Author", "YOP", "Edition", "Qty"]];
          values.map((item, index) => {
            if (
              index === 0 ||
              values[index - 1].subject_name !== item.subject_name
      ...
      if (reportType.value == "book_subject_wise_qty") {
        if (type == 1) {
          var csvData = [
            [
              { content: "No." },
          ...
          values.map((item, index) => {
            index === 0
              ...
              : data[index - 1].subject_name === item.subject_name
              ? (totalQty += item.quantity)
              : (totalQty = item.quantity);
            if (
              index === 0 ||
          ...
          var csvData = [["No.", "Title", "Author", "Year", "Edition", "Qty"]];
          values.map((item, index) => {
            index === 0
              ? (totalQty = item.quantity)
              : data[index - 1].subject_name === item.subject_name
                  ...
                  <div className="row no-gutters">
                    <div className="col-lg-6 pe-2">
                      ...
                      <ReactSelectField
                        ...
                        options={departmentBookReportList}
                        onChange={(text) => {
                          setReportType(text);
                          handleClear();
                        }}
                    ...
                    <>
                      <div className="row mt-4 border p-3">
                        ...
                        <div className="table-responsive mt-4 p-0">
                          ...
                          {reportType.value == "book_title_wise_qty" ? (
                            <table
                              className="table report-table table-bordered"
                              id="pdf-table"
                            >
                          ...
                          {reportType.value == "book_subject_wise_qty" ? (
                            <table
                              className="table report-table table-bordered"
                              id="pdf-table"
                            >
...
export default DepartmentBookDetailReport;
