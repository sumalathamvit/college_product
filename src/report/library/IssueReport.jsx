...
import { libraryIssueReportList } from "../../component/common/CommonArray";
...
function LibraryBookIssueReport() {
  const [load, setLoad] = useState(false);
  const [data, setData] = useState([]);
  const [showLoadMore, setShowLoadMore] = useState(false);

  const [reportType, setReportType] = useState(libraryIssueReportList[0]);
  ...
  const handleCSVData = async (values, type) => {
    ...
    try {
      if (
        reportType.value == "issue_day_wise" ||
        reportType.value == "issue_week_wise" ||
        reportType.value == "issue_month_wise"
      ) {
        if (type == 1) {
          var pdfData = [
            [
          ...
          values.map((item1, index1) => {
            ...
            if (reportType.value == "issue_day_wise") {
              issueRow.content =
                "Issue Date : " + moment(item1.issue_date).format("DD-MM-YYYY");
            } else if (reportType.value == "issue_week_wise") {
              issueRow.content =
                "Issue Week : " +
                moment(item1.issue_date).format("DD-MM-YYYY") +
                " to " +
                moment(item1.issue_date).add(6, "days").format("DD-MM-YYYY");
            } else if (reportType.value == "issue_month_wise") {
              issueRow.content =
                "Issue Month : " + moment(item1.issue_date).format("MMM-YYYY");
            }
          ...
          var columnWidth = [3, 6, 25, 15, 4, 6, 10, 18, 5, 7];
        } else {
      ...
      if (
        reportType.value == "issue_student_day_wise" ||
        reportType.value == "issue_student_week_wise" ||
        reportType.value == "issue_student_month_wise"
      ) {
        if (type == 1) {
          var pdfData = [
            [
          ...
          values.map((item1, index1) => {
            ...
            if (reportType.value == "issue_student_day_wise") {
              issueRow.content =
                "Issue Date : " + moment(item1.issue_date).format("DD-MM-YYYY");
            } else if (reportType.value == "issue_student_week_wise") {
              issueRow.content =
                "Issue Week : " +
                moment(item1.issue_date).format("DD-MM-YYYY") +
                " to " +
                moment(item1.issue_date).add(6, "days").format("DD-MM-YYYY");
            } else if (reportType.value == "issue_student_month_wise") {
              issueRow.content =
                "Issue Month : " + moment(item1.issue_date).format("MMM-YYYY");
            }
          ...
          values.map((item1, index1) => {
            ...
            const issueRow = [];
            issueRow[0] =
              "Issue Date : " + reportType.value == "issue_staff_week_wise"
                ? moment(item1.issue_date).format("DD-MM-YYYY") +
                  " " +
                  "to" +
                  " " +
                  moment(item1.issue_date).add(6, "days").format("DD-MM-YYYY")
                : reportType.value == "issue_staff_month_wise"
                ? moment(item1.issue_date).format("MM-YYYY")
                : moment(item1.issue_date).format("DD-MM-YYYY");
            for (let i = 1; i < 10; i++) {
              issueRow[i] = ""; // Leave other cells empty to achieve colspan effect
            }
      ...
      if (
        reportType.value == "issue_staff_day_wise" ||
        reportType.value == "issue_staff_week_wise" ||
        reportType.value == "issue_staff_month_wise"
      ) {
        if (type == 1) {
          var pdfData = [
          ...
          values.map((item1, index1) => {
            ...
            if (reportType.value == "issue_staff_day_wise") {
              issueRow.content =
                "Issue Date : " + moment(item1.issue_date).format("DD-MM-YYYY");
            } else if (reportType.value == "issue_staff_week_wise") {
              issueRow.content =
                "Issue Week : " +
                moment(item1.issue_date).format("DD-MM-YYYY") +
                " to " +
                moment(item1.issue_date).add(6, "days").format("DD-MM-YYYY");
            } else if (reportType.value == "issue_staff_month_wise") {
              issueRow.content =
                "Issue Month : " + moment(item1.issue_date).format("MMM-YYYY");
            }
  ...
  const getBookDetailList = async (values, showAll = 0, report = 0) => {
    ...
    try {
      setLoad(true);
      const getBookDetail = await libraryApi.getIssueReportSearch(
      ...
      if (report) {
        if (
          ...
          const filterList = preFunction.handleDayWise(
            getBookDetail.data.message,
            report
          );
        ...
        if (
          ...
          const filterList = preFunction.handleWeekWise(
            getBookDetail.data.message,
            report
          );
          handleCSVData(filterList, report);
        ...
        if (
          ...
          const filterList = preFunction.handleMonthWise(
            getBookDetail.data.message,
            report
          );
          handleCSVData(filterList, report);
                  ...
                  <div className="row no-gutters">
                    <div className="col-lg-6 pe-2">
                      ...
                      <ReactSelectField
                        ...
                        options={libraryIssueReportList}
                        onChange={(text) => {
                          setReportType(text);
                          handleClear();
                        }}
...
export default LibraryBookIssueReport;
