...
function OnHandBookAccessNoWise() {
  let totalQty = 0;
  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (type == 1) {
        ...
        var columnWidth = [3, 10, 35, 20, 20, 5, 7];
      } else {
        var csvData = [["No."]];
      ...
      if (type == 1) {
        preFunction.generatePDF(
          ...
          "On Hand Book Details Access No. Wise",
          pdfData,
          columnWidth
        );
      } else {
        preFunction.downloadCSV(
          csvData,
          "On Hand Book Details Access No. Wise" + ".csv"
        );
      }
      setLoad(false);
    } catch (error) {
...
export default OnHandBookAccessNoWise;
