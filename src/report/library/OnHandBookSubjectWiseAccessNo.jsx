...
function OnHandBookSubjectWiseAccessNo() {
  let totalQty = 0;
    ...
    try {
      ...
      if (type == 1) {
        preFunction.generatePDF(
          ...
          "On Hand Book Details Subject Wise With Access Number",
          pdfData,
          columnWidth
        );
      } else {
        preFunction.downloadCSV(
          csvData,
          "On Hand Book Details Subject Wise With Access Number.csv"
        );
      }
      setLoad(false);
    } catch (error) {
  ...
  const getBookDetailList = async (values, showAll = 0, report = 0) => {
    ...
    try {
      ...
      const onHandBookRes = await libraryApi.getOnHandBook(
        ...
        "book_subject_wise_access_number",
...
export default OnHandBookSubjectWiseAccessNo;
