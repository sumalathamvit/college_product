...
function OnHandBookTitleWiseQty() {
  let totalQty = 0;
  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (type == 1) {
        ...
        var columnWidth = [3, 35, 20, 20, 7, 10, 5];
      } else {
        var csvData = [["No."]];
        csvData[0].push(
          "Title",
      ...
      if (type == 1) {
        preFunction.generatePDF(
          ...
          "On Hand Book Details Title Wise Quantity",
          pdfData,
          columnWidth
        );
      } else {
        preFunction.downloadCSV(
          csvData,
          "On Hand Book Details Title Wise Quantity.csv"
        );
      }
      setLoad(false);
    } catch (error) {
...
export default OnHandBookTitleWiseQty;
