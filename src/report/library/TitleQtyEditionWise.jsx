  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (type == 1) {
        ...
        var csvData = [["No.", "Title", "Author", "Edition", "YOP", "Qty"]];
        values.map((item, index) => {
          if (
            index === 0 ||
            values[index - 1].subject_name !== item.subject_name
        ...
        preFunction.downloadCSV(csvData, "Title Qty Edition Wise");
      }
      setLoad(false);
    } catch (error) {
      console.log(error);
...
export default TitleQtyEditionWise;
