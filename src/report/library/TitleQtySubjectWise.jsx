  ...
  const handleCSVData = async (values, type) => {
    try {
      ...
      if (type == 1) {
        ...
        var csvData = [
          ["No.", "Title", "Author Name", "Edition", "YOP", "Qty"],
        ];
        values.map((item, index) => {
          index > 0 && data[index - 1].subject_name === item.subject_name
            ? (totalQty += item.quantity)
        ...
        preFunction.downloadCSV(csvData, "Title Qty Subject Wise");
      }
      setLoad(false);
    } catch (error) {
      console.log(error);
...
export default TitleQtySubjectWise;
