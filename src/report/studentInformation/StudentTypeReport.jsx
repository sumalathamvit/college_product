...
function StudentTypeReport() {
  ...
  const handleCSVData = async (exportData, type, values) => {
    ...
    try {
      ...
      if (type === 2) {
        ...
        var head1 = [
          [
            ...
            { content: "Reg. No", styles: topLineStyle },
            { content: "Name", styles: topLineStyle },
            { content: "Admission Type", styles: topLineStyle },
            { content: RENAME?.section, styles: topLineStyle },
      ...
      if (type === 2) {
        preFunction.downloadCSV(pdfData, "Student Type Report.csv");
      } else {
  ...
  const getAllList = async (collegeId, course) => {
    try {
      ...
      // setCategoryList(masterRes8.data.message.data.category);
    } catch (error) {
      console.log("error", error);
    }
  };
  ...
  // const handleSearchStaff = async (text) => {
  //   if (text.replace(/\s\s+/g, " ").trim().length > 2) {
  //     try {
  //       const staffRes = await empApi.employeeSearch(text);
  //       console.log("staffRes--------", staffRes);
  //       setStaffList(staffRes.data.message.employee_data);
  //     } catch (error) {
  //       console.log("error--", error);
  //     }
  //   } else {
...
export default StudentTypeReport;
